...
GI QuarticResSymbol(GI alpha, GI beta, bool output)
// calculates the quartic residue symbol (alpha, beta)_4 where beta is an odd Gaussian integer
{
    ...
    while (true) {
        if (output) cout << "$\\biq{" << alpha << "}" << "{" << beta << "}=";
        beta = GIprimary(beta);
        if (output) cout << "\\biq{" << alpha << "}" << "{" << beta << "}$ & ";

        if (output) cout << "$" << alpha << "=(" << beta << ")" << "(" << alpha / beta << ")+" << "(" << alpha % beta << ")$ & ";
        ...
        while (alpha % GI(1, 1) == GI(0)) {          // remove factors of 1 + i and apply
            ...
            resp *= GIpow(GI(0, 1), ((beta.real() - beta.imag() - beta.imag() * beta.imag() - 1) / 4) % 4 + 4);
            cnt++;
        }

        if (output) cout << "\\biq{" << "(1 + \\i)^" << cnt << "(" << alpha << ")}" << "{" << beta << "}="
            << resp << "\\times\\biq{" << alpha << "}" << "{" << beta << "}=";

        GI u = alpha / GIprimary(alpha);              // the inverse of the unit for the primary associate

        if (output) cout << resp << "\\times\\biq{" << u << "}" << "{" << beta << "}\\biq{" << GIprimary(alpha) << "}" << "{" << beta << "}=";
        if (u == GI(-1)) resp *= GIpow(GI(0, 1), (1 - beta.real()) % 4 + 4);
        if (u == GI(0, 1)) resp *= GIpow(GI(0, 1), ((1 - beta.real()) / 2) % 4 + 4);
        if (u == GI(0, -1)) resp *= GIpow(GI(0, 1), (3 * (1 - beta.real()) / 2) % 4 + 4);
        alpha = GIprimary(alpha);
        if (output) cout << resp << "\\times\\biq{" << alpha << "}" << "{" << beta << "}=";
        ...
        {
            if (leap)
                cout << resp << "\\times -1 \\times\\biq{" << beta << "}" << "{" << alpha << "}$";
            else
                cout << resp << "\\times\\biq{" << beta << "}" << "{" << alpha << "}$";
        }
        swap(alpha, beta);
        if (output) cout << "\\\\ \\hline" << endl;
    }
}